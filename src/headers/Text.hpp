...
#define initialDefaultTextScale sf::Vector2f(0.5, 0.5)
...
class Text
{
	...
	Text()
	{
		...
		if (pixelated) { pixelateFont(font, fontSize, fontPath); }
		setDefaultTextColor(initialDefaultTextColor);
		setDefaultTextScale(initialDefaultTextScale);
	}

	void pixelateFont(sf::Font &font, int fontSize, const char fontPath[])
	{
